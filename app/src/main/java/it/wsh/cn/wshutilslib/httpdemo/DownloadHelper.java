...
import it.wsh.cn.wshutilslib.httpdemo.providers.downloads.Downloads;
...
public class DownloadHelper {
    ...
    public static class DownloadChangeObserver extends ContentObserver {
        ...
        public void onChange(boolean selfChange) {
            ...
            DownloadHelper.getInstance().sendDownloadProcess();
        }
    }
    ...
    public void  pause(long... ids) {
        //mDownloadManager.remove(id);
        Cursor cursor = mDownloadManager.query(new DownloadManager.Query().setFilterById(ids));
        try {
        ...
        //values.put(Downloads.COLUMN_STATUS, Downloads.CONTROL_PAUSED);
        values.put(Downloads.COLUMN_NO_INTEGRITY, 1);
        try {
            Field baseUriField = DownloadManager.class.getDeclaredField("mBaseUri");
            baseUriField.setAccessible(true);
            Uri baseUri = (Uri) baseUriField.get(mDownloadManager);

            Method getWhereClauseForIdsMethod = DownloadManager.class.getDeclaredMethod("getWhereClauseForIds", long[].class);
            getWhereClauseForIdsMethod.setAccessible(true);
            String whereClause = (String) getWhereClauseForIdsMethod.invoke(mDownloadManager, ids);

            Method getWhereArgsForIdsMethod = DownloadManager.class.getDeclaredMethod("getWhereArgsForIds", long[].class);
            getWhereArgsForIdsMethod.setAccessible(true);
            String[] whereArgs = (String[]) getWhereArgsForIdsMethod.invoke(mDownloadManager, ids);

            mContext.getContentResolver().update(baseUri, values, whereClause, whereArgs);
        } catch (Exception e) {
            e.printStackTrace();
        }
